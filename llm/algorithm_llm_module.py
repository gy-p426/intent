...
- anomaly: 异常检测，发现数据中的异常点，关键词：异常点、异常值、异常波动、离群点、outlier、检测异常、发现异常
- associate: 关联分析，发现数据项之间的关联规则，关键词：关联、关系、规则
- compare: 对比分析，比较不同组别或时期的差异，关键词：对比、比较、差异、A/B测试
- similarity: 相似度分析，计算实体间相似程度，关键词：相似、相似度、匹配、相近
- trend: 趋势分析，分析时间序列的趋势变化，关键词：趋势、变化趋势、走势、季节性、趋势分析
...
识别原则：
1. 仔细分析用户问题的核心动作和目标
2. 重点关注问题中的动词（如：检测、分析、预测、查询）和目标名词（如：异常点、趋势、未来值）
3. 从候选算法类型中选择最匹配的一个
4. 当问题同时包含多个可能的算法特征时，优先选择与核心目标最匹配的算法
...
问题："分析每日出车次数的异常点" → 输出：anomaly（核心目标是找异常点）
问题："查询每天异常车辆个数的趋势" → 输出：trend（核心目标是分析趋势）
问题："分析商品关联关系" → 输出：associate
问题："对比两个季度业绩" → 输出：compare
问题："分析销售趋势变化" → 输出：trend
"""
