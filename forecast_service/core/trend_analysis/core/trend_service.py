...
class TrendService:
    ...
    def _generate_decomposition_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        seasonal_strength = metrics.get("seasonal_strength", 0)
        algorithm = metrics.get("algorithm", "未知")
        period = metrics.get("period_used", 0)
        ...
        summary["key_findings"].append(trend_finding)
        ...
        summary["key_findings"].append(seasonal_finding)
        ...
        if trend_strength >= 0.5:
            summary["recommendations"].append("💡 趋势明显，建议关注长期变化方向，可能需要调整策略以适应趋势")
        if seasonal_strength >= 0.4:
            summary["recommendations"].append(f"💡 周期性明显（周期约{period}个单位），建议在业务规划中考虑这种周期性波动")
        if trend_strength < 0.3 and seasonal_strength < 0.2:
            summary["recommendations"].append("💡 数据较为平稳，没有明显的趋势或周期性，可能受随机因素影响较大")
    ...
    def _generate_detection_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        significant = results.get("statistical_significance", False)
        p_value = results.get("p_value", 1.0)
        confidence = results.get("confidence_level", 0.95)
        method = results.get("method", "未知")
        ...
        summary["key_findings"].append(main_finding)
        
        # 添加斜率解读（如果有）
        slope = results.get("slope") or results.get("sen_slope")
        if slope is not None and significant:
            ...
            summary["key_findings"].append(slope_finding)
        ...
        if r_squared is not None:
            ...
            summary["key_findings"].append(r2_finding)
        ...
        if significant:
            if direction == "increasing":
                summary["recommendations"].append("💡 数据呈上升趋势，建议关注增长的可持续性，并分析增长原因")
                summary["recommendations"].append("💡 可以考虑利用这一趋势进行预测和规划")
            elif direction == "decreasing":
                summary["recommendations"].append("💡 数据呈下降趋势，建议分析下降原因，评估是否需要采取措施")
                summary["recommendations"].append("💡 关注下降速度，判断是否会影响业务目标")
        else:
            summary["recommendations"].append("💡 数据较为平稳，可以继续监测，观察是否会出现新的趋势")
            summary["recommendations"].append("💡 如果期望看到变化，可能需要采取主动措施来推动")
