...
class AlgorithmExecutor(IAlgorithmExecutor):
    ...
    async def execute_similarity(
        ...
        Returns:
            AlgorithmExecutionResponse: 执行响应
        """
        logger.info("开始执行相似度分析算法")
        logger.info(f"输入数据行数: {len(request.data_rows)}")
        logger.info(f"相似度分析配置: {request.config}")

        try:
            # 调用相似度分析API
            result_data = await self.algorithm_client.call_similarity_api(
                data_rows=request.data_rows,
                config=request.config
            )

            logger.info("相似度分析算法执行成功")

            # 检查响应是否包含错误信息
            if 'error' in result_data:
                error_msg = result_data.get('error', '未知错误')
                logger.error(f"相似度分析返回错误: {error_msg}")
                return AlgorithmExecutionResponse(
                    result=result_data,
                    status="error",
                    message=f"相似度分析执行失败: {error_msg}",
                    readable_result=None
                )
            ...
            has_valid_result = (
                result_data.get('status') == 'success' or
                'similarity_score' in result_data or
                'dtw_distance' in result_data or
                ('解释' in result_data and 'error' not in result_data)
            )
            ...
            if has_valid_result and result_data:
                ...
                logger.info(f"相似度分析执行成功，响应包含字段: {list(result_data.keys())}")
                ...
                error_msg = result_data.get('message', '响应格式不符合预期')
                logger.error(f"相似度分析返回未知格式: {result_data}")
                return AlgorithmExecutionResponse(
                    result=result_data,
                    status="error",
                    message=f"相似度分析执行失败: {error_msg}",
            ...
            logger.error(f"相似度分析执行失败: {str(e)}")
            return AlgorithmExecutionResponse(
                result={},
                status="error",
                message=f"相似度分析执行失败: {str(e)}",
                readable_result=None
            )
