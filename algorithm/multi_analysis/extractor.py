...
5. 优先选择有注释说明的列
6. 🚨 重要：normalized_query中必须明确要求聚合操作，避免返回重复数据！格式如"按月汇总2024年8月和2025年8月的出车数据，获取月份、出车次数总和，共2列数据"！！！
7. required_columns中一定写明列注释，一定与normalized_query的使用的名称相同
8. normalized_query必须包含扩展后的完整时间范围描述（如"2025年7月至8月"而非仅"8月"）
9. 🚨 关键：为避免SQL缺少GROUP BY导致重复数据，normalized_query必须包含"汇总"、"按月统计"、"聚合"等聚合关键词
...
{{
  ...
  "required_columns": ["日期", "销售额"],
  "normalized_query": "按年汇总2020年至2025年的销售数据，获取年份、销售额总和，共2列数据"
}}
