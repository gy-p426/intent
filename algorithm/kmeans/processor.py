"""
K-Means Algorithm Data Processor
...
class KMeansProcessor(BaseAlgorithmProcessor):
    """K-Means聚类算法数据处理器"""
    ...
    async def convert_sql_result_to_algorithm_input(
        ...
        """将SQL结果转换为K-Means算法输入格式"""
        try:
            logger.info(f"开始转换SQL结果为K-Means算法输入，数据行数: {len(sql_result)}")
            ...
            logger.info(f"K-Means数据转换完成，配置: {config}")
            ...
            logger.error(f"K-Means数据转换失败: {str(e)}")
            raise ValueError(f"K-Means数据转换失败: {str(e)}")
    ...
    async def validate_algorithm_input(
        ...
        try:
            ...
            if k_value is not None:
                if k_value >= len(data_rows):
                    logger.error(f"K值({k_value})不能大于等于数据行数({len(data_rows)})")
                    return False
            
            logger.info("K-Means算法输入验证通过")
            return True
            
        except Exception as e:
            logger.error(f"K-Means输入验证失败: {str(e)}")
            return False
