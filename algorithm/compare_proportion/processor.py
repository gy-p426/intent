...
class CompareProportionProcessor(BaseAlgorithmProcessor):
    ...
    async def convert_sql_result_to_algorithm_input(
        ...
        """将SQL结果转换为占比分析算法输入格式"""
        try:
            logger.info(f"开始转换SQL结果为占比分析算法输入，数据行数: {len(sql_result)}")
            ...
            logger.info(f"占比分析数据转换完成，配置: {config}")
            ...
            logger.error(f"占比分析数据转换失败: {str(e)}")
            raise ValueError(f"占比分析数据转换失败: {str(e)}")
    ...
    async def validate_algorithm_input(
        ...
        try:
            ...
            logger.info("占比分析算法输入验证通过")
            return True

        except Exception as e:
            logger.error(f"占比分析算法输入验证失败: {str(e)}")
            return False
