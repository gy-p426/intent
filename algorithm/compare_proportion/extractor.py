...
class CompareProportionExtractor(BaseAlgorithmExtractor):
    ...
    def algorithm_name(self) -> str:
        return "compare_proportion"  # 保留具体算法名称以便区分
    ...
    async def build_extraction_prompt(
        ...
        system_prompt = f"""你是比较分析（占比分析）分析专家。根据用户问题和数据库信息，提取占比分析所需的参数。
...
6. normalized_query中一定写明返回的数据列注释（即id_column+feature_columns），并且标名返回几列数据，否则无法正确解析，如"获取销售日期、销售额，返回销售日期、销售额共2列数据"，！！！
...
请严格按照系统提示的规则分析用户需求，输出符合占比分析要求的JSON参数。
...
输出JSON格式的参数提取结果。"""
    ...
    def parse_extraction_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"占比分析参数解析失败: {str(e)}")
            raise ValueError(f"占比分析参数解析失败: {str(e)}")
