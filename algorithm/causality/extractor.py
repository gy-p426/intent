...
2. independent_variables的值必须是数据库中实际存在的数值型列注释列表
   ...
   - 例如:如果因变量是"销售额",除了用户提到的字段外,还应该考虑:
     营销相关:广告投入、促销活动、营销费用等
     产品相关:价格、库存、产品数量等
     时间相关:月份、季度、年份等
     地理相关:区域、城市等(如果有数值编码)
     其他业务指标:客户数量、访问量、转化率等
3. 不要创造不存在的列注释
...
格式示例说明:
1. 列的顺序:因变量必须放在第一位,然后是所有自变量
    示例:["销售额", "广告投入", "促销活动", ...] - 销售额是因变量,放在第一位
   
2. required_columns:必须包含所有列(因变量+自变量),顺序与parameter_mapping一致
...
3. normalized_query格式:必须明确说明返回的列和列数
    ...
    示例1:"获取2023年1月到2023年12月期间的销售数据的销售额、广告投入、价格,返回销售额、广告投入、价格共3列数据"
    示例2:"获取历史销售数据的销售额、广告投入、促销活动、季节、价格、库存、客户数量、访问量,返回销售额、广告投入、促销活动、季节、价格、库存、客户数量、访问量共8列数据"
...
2. 主动从数据库schema中寻找所有可能影响因变量的数值型字段作为自变量
   - 不要只选择用户明确提到的字段
   - 要全面分析数据库中所有可能相关的数值型字段
   - 考虑业务逻辑,包含所有潜在的影响因素
   - 字段数量不限,尽可能全面
3. 所有列注释必须与数据库中的实际列注释完全匹配
4. **normalized_query格式要求(重要)**:
   - 必须写明返回的所有列名(因变量+所有自变量)
   - 必须标明返回的总列数
   - 格式:"获取[条件]的[列1]、[列2]、[列3]...,返回[列1]、[列2]、[列3]...共N列数据"
   - 示例:"获取2023年销售数据的销售额、广告投入、价格,返回销售额、广告投入、价格共3列数据"
5. **列的顺序**:因变量必须放在第一位,然后是所有自变量
6. **列名一致性**:parameter_mapping、required_columns、normalized_query中的列名必须完全一致
