...
class CreateFavoriteRequest(BaseModel):
    ...
    algorithm_params: Optional[dict] = Field(None, description="算法参数")
    original_sql: str = Field(..., description="原始SQL")
    category_id: int = Field(..., description="目录ID")
...
class UpdateFavoriteRequest(BaseModel):
    ...
    category_id: Optional[int] = Field(None, description="目录ID")
    description: Optional[str] = Field(None, description="描述")
...
async def update_favorite(
    ...
    try:
        service = FavoriteService()
        success = service.update_favorite(
            request.favorite_id,
            request.dict(exclude={'favorite_id'}, exclude_none=True)
        )
...
async def delete_favorites(
    ...
    try:
        service = FavoriteService()
        result = service.delete_favorites(request.favorite_ids)
